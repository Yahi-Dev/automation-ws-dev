// src/features/posts/components/post-update.tsx
"use client"
...
import { useUpdatePost, usePostById, usePostImages } from "../hooks/use-posts"
...
export function EditPostForm({ id }: EditPostFormProps) {
  ...
  const { post, loading: loadingPost } = usePostById(id)
  const { update, isLoading, error, clearError } = useUpdatePost(id)
  const { uploadImage, isUploading } = usePostImages()

  const [formData, setFormData] = useState<{ schedule: string; text: string }>({
    schedule: "",
    text: "",
  })

  const [images, setImages] = useState<{ url: string; file?: File; isNew?: boolean }[]>([])
  const [errors, setErrors] = useState<Partial<{ schedule: string; text: string; images?: string }>>({})
  ...
  useEffect(() => {
    if (post) {
      setFormData({
        schedule: format(new Date(post.schedule), "yyyy-MM-dd'T'HH:mm"),
        text: post.text || "",
      })
      setImages(post.images?.map(img => ({ url: img.url })) || [])
    }
  }, [post])
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (images.length >= 1) {
      setErrors(prev => ({ 
        ...prev, 
        images: "Solo puedes tener una imagen por post" 
      }))
      // Limpiar input
      e.target.value = ""
      return
    ...
    try {
      ...
      setImages(prev => [...prev, { url: imageUrl, file, isNew: true }])
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.schedule.trim()) {
      ...
      if (selectedDate <= now) {
        ...
        toast.error("Fecha no válida", {
          description: `La fecha seleccionada ya pasó ${timeAgo}. Elige una fecha futura para reprogramar el post.`,
          duration: 5000,
        })
      }
    }
    ...
    if (!formData.text.trim()) {
      ...
      newErrors.text = "El texto no puede exceder 1000 caracteres"
    }

    // Validar máximo una imagen
    if (images.length > 1) {
      newErrors.images = "Solo se permite una imagen por post"
    }
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-500">
                  Selecciona una fecha y hora futura para reprogramar la publicación
                </p>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <Input
                      ...
                      disabled={isLoading || isUploading || images.length >= 1} // ← Deshabilitar cuando hay imagen
                      className="flex-1"
                    />
                    <Button
                      type="button"
                    ...
                    >
                      ...
                      {isUploading ? "Subiendo..." : images.length === 0 ? "Seleccionar" : "Reemplazar"}
                    </Button>
                  </div>
                  ...
                  {images.length > 0 && (
                    <div className="space-y-3">
                      <Label className="text-sm font-medium">
                        {images.some(img => img.isNew) ? "Nueva imagen seleccionada:" : "Imagen actual:"}
                      </Label>
                  ...
                  <p className="text-sm text-gray-500">
                    Solo puedes tener una imagen por post. Formatos soportados: JPG, PNG, GIF. Tamaño máximo: 5MB.
                  </p>
                </div>
              </div>
            </div>
